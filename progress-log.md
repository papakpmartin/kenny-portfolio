# Kenny Portfolio — Progress Log

Historical implementation/progress notes moved out of `task-queue.md` so the queue can stay focused on active work.

- 2026-04-18: Refreshed Kenny job-search materials by adding `job-search/materials-refresh-2026-04-18.md`, tightening target-role positioning in `job-search/tracker.md`, and moving site resume content into `site/src/data/resume.js` so the `/resume` page has a single structured source of truth.
...
- 2026-03-11: Added LA-only per-unit style controls (`widthMode: full|inset`, `gapMode: tight`) with supporting CSS classes; applied to LA modules to tighten intra-unit spacing and introduce reusable full/inset width behavior.
